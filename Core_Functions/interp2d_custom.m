function outputImage = interp2d_custom(inputImage,X,Y,method,fillValues, varargin)%#codegen
...
if useIpp
    ...
    remapFuncSwitch = true; %% remapmex exist
    ...
    if release_yr_image >= 2019
        if strcmpi(release_type, 'a')
            remapFuncSwitch = false;
        else
            remapFuncSwitch = true;
        end
    end
    
    if remapFuncSwitch
        if isreal(inputImage)
            outputImage = images.internal.coder.remapmex(inputImage,X,Y,method,fillValues);
        else
            outputImage = complex(images.internal.coder.remapmex(real(inputImage),X,Y,method,real(fillValues)),...
                images.internal.coder.remapmex(imag(inputImage),X,Y,method,imag(fillValues)));
        end
    else
        if isreal(inputImage)
