function [outputImage,outputRef] = imwarp_custom(varargin)
...
outputImage = remapPointsAndResample(parsedInputs.InputImage,R_A,tform,outputRef,method,fillValues, SmoothEdges,  varargin{7});

outputImage = cast(outputImage,'like',parsedInputs.InputImage);

function outputImage = remapPointsAndResample(inputImage,R_A,tform,outputRef,method,fillValues, SmoothEdges, resc_warp)
...
if tform.Dimensionality ==2
    ...
    if useIPP
        outputImage = ippWarpAffine(inputImage,R_A,tform,outputRef,method,fillValues, SmoothEdges, resc_warp);
    elseif isa(tform,'affine2d') || isa(tform,'projective2d')
        outputImage = remapAndResampleInvertible2d(inputImage,R_A,tform,outputRef,method,fillValues, SmoothEdges, resc_warp);
    else
        outputImage = remapAndResampleGeneric2d(inputImage,R_A,tform,outputRef,method,fillValues, SmoothEdges, resc_warp);
    end
...
parser.addParameter('WarpRescale',[]);
...
function outputImage = remapAndResampleGeneric2d(inputImage,R_A,tform,outputRef,method,fillValues, SmoothEdges, resc_warp)
...
original_size=size(dstXWorld);
% % % % % resc_warp=varargin{7};

dstXWorld=imresize(dstXWorld,resc_warp);
dstYWorld=imresize(dstYWorld,resc_warp);

[srcXWorld,srcYWorld] = tform.transformPointsInverse(dstXWorld,dstYWorld);

srcXWorld=imresize(srcXWorld,original_size);
srcYWorld=imresize(srcYWorld,original_size);

clear dstXWorld dstYWorld resc_warp original_size
...
%%% WC EDIT -- version check
interp2d_Switch = true; %% interp2d of 2019b toolbox exist
...
release_type = toolbox_Release{Index_image}(end-1);
if release_yr_image >= 2019
    if strcmpi(release_type, 'a')
        interp2d_Switch = false;
    else
        interp2d_Switch = true;
    end
end
...
if interp2d_Switch
    ...
    outputImage = interp2d_custom(inputImage,srcXIntrinsic,srcYIntrinsic,method,fillValues, SmoothEdges);
end
